...
{
	out.print("<script>javascript:alert('Sorry, Login first£¡');history.back();</script>");
	out.close();
}
%>
<html>
  ...
  <body >
  <% 
  String lb=request.getParameter(&quot;lb&quot;); %>Edit<%=lb%>:
  <br><br>
  <%
...
HashMap m = new CommDAO().getmaps("leibie",lb,"dx");
  ...
  <form  action="dx.jsp?f=f&id=<%=m.get("id")%>&lb=<%=lb%>"  method="post" name="f1" >
   <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF" style="border-collapse:collapse">  
     ...
     <tr><td>Content£º</td><td><textarea name="content" cols="100" rows="8" style="width:700px;height:400px;visibility:hidden;"><%=m.get("content")%></textarea>
			<input name="lb" type="hidden" id="lb" value="<%=m.get("leibie")%>"></td></tr>
     
    <tr>
      <td>&nbsp;</td>
      <td><input type="submit" name="Submit" value="Save" onClick="return check();" />
