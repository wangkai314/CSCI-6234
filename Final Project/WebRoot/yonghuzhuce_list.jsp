...
<html>
  <head>
    <title>Users</title><LINK href="css.css" type=text/css rel=stylesheet>
  </head>
  <body >
  <p>Users List£º</p>
  <form name="form1" id="form1" method="post" action="">
   Find: UserName£º<input name="yonghuming" type="text" id="yonghuming" size="12" /> 
   Name£º<input name="xingming" type="text" id="xingming" size="12" /> 
   FB£º<input name="QQ" type="text" id="QQ" size="12" /> 
   E-mail£º<input name="youxiang" type="text" id="youxiang" size="12" /> 
   Phone£º<input name="dianhua" type="text" id="dianhua" size="12" /> 
   ID£º<input name="shenfenzheng" type="text" id="shenfenzheng" size="15" />
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
  ...
  <tr>
    ...
    <td><%=map.get("yonghuming") %></td><td><%=map.get("mima") %></td><td><%=map.get("xingming") %></td><td><%=map.get("xingbie").equals("ÄÐ") ? "Male" : "Female" %></td><td><%=map.get("chushengnianyue") %></td><td><%=map.get("QQ") %></td><td><%=map.get("youxiang") %></td><td><%=map.get("dianhua") %></td><td><%=map.get("shenfenzheng") %></td><td><a href='<%=map.get("touxiang") %>' target='_blank'><img src='<%=map.get("touxiang") %>' width=88 height=99 border=0 /></a></td><td><%=map.get("dizhi") %></td>
    <td width="94" align="center"><%=map.get("addtime") %></td>
    <td width="52" align="center"><a href="sh.jsp?id=<%=map.get("id")%>&yuan=<%=map.get("issh")%>&tablename=yonghuzhuce" onClick="return confirm('You Sure£¿');"><%=map.get("issh").equals("ÊÇ") ? "Yse" : "No"%></a></td>
    <td width="71" align="center"><a href="yonghuzhuce_updt.jsp?id=<%=map.get("id")%>">Modify</a>  <a href="yonghuzhuce_list.jsp?scid=<%=map.get("id") %>" onClick="return confirm('You sure£¿')">Delete</a></td>
  </tr>
  	<%
